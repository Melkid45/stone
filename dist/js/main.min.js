...
function raf(time) {
    lenis.raf(time);
    if (time % 2 === 0) ScrollTrigger.update();
...
if (isTouch) {
    ...
    $('.work video').remove()
    $('.services-frame-item .wrap-services-video').remove()
} else {
    $('.services-frame-item .wrap-services-images').remove()
...
if (!isTouch) {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const generatedPosters = new Map();

        // ====== Генерация постера ======
        function generatePoster(src) {
            return new Promise((resolve, reject) => {
                if (generatedPosters.has(src)) return resolve(generatedPosters.get(src));
                ...
                tempVideo.addEventListener("loadeddata", () => {
                    try {
                        ...
                        generatedPosters.set(src, dataURL);
                        resolve(dataURL);
                    } catch (e) {
                        reject(e);
                    }
        ...
        function attachPoster(video) {
            const source = video.querySelector('source');
            const src = source?.dataset.src || video.src;
            if (!src) return;

            // Постеры генерируем на всех устройствах, но на мобилке видео src не трогаем
            generatePoster(src).then(url => {
