...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.generatePosterFromVideo = function (video) {
        return new Promise((resolve, reject) => {
            if (video.dataset.posterGenerated) {
                resolve(video.getAttribute('poster'));
                return;
            }

            if (!video.videoWidth || !video.videoHeight) {
                reject("Нет размеров видео для постера");
                return;
            }
    ...
    function getOrInitVideo(source) {
    ...
    function initItemVideo(videoElem) {
        const source = videoElem.querySelector('source');
        if (!source) return;
        const cachedVideo = getOrInitVideo(source);
        if (!videoElem.dataset.initialized) {
            videoElem.dataset.initialized = "true";
            // Подставляем poster сразу
            generatePosterFromVideo(cachedVideo).then(url => videoElem.setAttribute('poster', url)).catch(()=>{});
        }
        videoElem.cachedVideo = cachedVideo;
    }

    function playVideo(item, videoElem) {
        const video = videoElem.cachedVideo;
        if (item.activeVideo && item.activeVideo !== video) pauseVideo(item, item.activeVideo);

        safePlay(video);
        videoElem.classList.add('show-video');
        item.setAttribute('data-video-playing', 'true');
        item.activeVideo = video;
    }
    ...
    function pauseVideo(item, video) {
        ...
        // Оставляем poster, не сбрасываем src
    }
    ...
    workItems.forEach(item => {
        ...
        videos.forEach(video => initItemVideo(video));

        if (!isTouch) {
            item.addEventListener('mouseenter', () => {
                const current = item.querySelector('.show-video') || videos[0];
                playVideo(item, current);
            });
            item.addEventListener('mouseleave', () => {
                if (item.activeVideo) pauseVideo(item, item.activeVideo);
            });
        }
    });

    // === Мобильный: IntersectionObserver ===
    if (isTouch) {
        const observer = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                const item = entry.target;
                const videoElem = item.querySelector('video.show-video') || item.querySelector('video');
                if (!videoElem) return;

                if (entry.isIntersecting) safePlay(videoElem.cachedVideo);
                else videoElem.cachedVideo.pause();
            });
    ...
    $('.information-tags-tag').on('click', function () {
        ...
        videos.forEach((vid, i) => {
            if (i === index) {
                ...
                initItemVideo(vid);
                safePlay(vid.cachedVideo);
            } else {
                vid.classList.remove('show-video');
                pauseVideo(frameItem, vid.cachedVideo);
            }
        });
    });
});
