...
$('.header-menu-open').off('click').on('click', function () {
    if ($('.header__menu').hasClass('opened') && OpenFlug) {
        lenis.start();
    } else if (!$('.header__menu').hasClass('opened') && OpenFlug) {
        lenis.stop();
    }
    ...
    if (OpenFlug) {
        ...
        setTimeout(() => { OpenFlug = true; }, 600); // уменьшил время
    }
});
...
if (isTouch) {
    ...
    $('.services-frame-item img').remove()
    $('.our-services-frame .item img').remove()
    $('.project-wrap .item img').remove()
}
...
if (!isTouch) {
        ...
        function initVideosForItem(item) {
            ...
            videos.forEach(video => {
                ...
                if (!isTouch && src && !video.src) {
                    // На десктопе присваиваем src, на мобилке не трогаем
                    video.src = src;
    ...
    $('.information-tags-tag').on('click', function (e) {
        let Index = $(this).index()
        $(this).addClass('current')
        $(this).addClass('current').siblings().removeClass('current');
        let FrameItem = $(this).closest('.services-frame-item')[0];
        let Images = FrameItem.querySelectorAll('img')
        Images.forEach((image, i) => {
            if (i === Index) {
                $(image).addClass('current_img')
            } else {
                $(image).removeClass('current_img')
            }
        })
    })
}
