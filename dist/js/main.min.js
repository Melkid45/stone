...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // === Функция генерации постера (Safari-safe) ===
    function generatePosterFromVideo(video) {
        return new Promise((resolve, reject) => {
            ...
            // Safari fix: использовать requestVideoFrameCallback, если доступен
    ...
    workItems.forEach(item => {
        ...
        const playVideo = (video) => {
            if (activeVideo && activeVideo !== video) {
                activeVideo.pause();
                activeVideo.classList.remove('show-video');
            }
            video.muted = true;
            safePlay(video);
    ...
    // === Мобильный: сначала постеры, потом IntersectionObserver ===
    if (isTouch) {
        // Ставим fallback постеры сразу
        workItems.forEach(item => {
            const videos = item.querySelectorAll('.preview-video, .services-video');
            videos.forEach(video => {
                const source = video.querySelector('source');
                if (source && source.dataset.src) {
                    // fallback-картинка, если указана
                    if (video.dataset.fallback) {
                        video.setAttribute("poster", video.dataset.fallback);
                    }

                    // грузим метаданные во временное видео → делаем постер
                    const temp = document.createElement('video');
                    temp.src = source.dataset.src;
                    temp.preload = "metadata";
                    temp.muted = true;

                    temp.addEventListener("loadeddata", () => {
                        generatePosterFromVideo(temp).then(url => {
                            video.setAttribute("poster", url);
                        }).catch(() => { });
                    }, { once: true });
                }
        ...
        // IntersectionObserver для реальной загрузки и воспроизведения
        const playObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                const item = entry.target;
                const video = item.querySelector('.show-video');
                if (!video) return;
        ...
        workItems.forEach(item => playObserver.observe(item));
    }
    ...
    $('.information-tags-tag').on('click', function () {
        ...
        const frameItem = $(this).closest('.services-frame-item');
        const videos = frameItem.find('.wrap-services-video video');
...
if (document.querySelector('.preloader')) {
    ...
    function updateProgress(percent) {
        ...
        // двигаем clip-path от 100% к 0%
        document.querySelector('.last-text').style.clipPath = `inset(0 ${100 - percent}% 0 0)`;
    }
    ...
    // Считаем прогресс загрузки ресурсов
    function trackLoadingProgress() {
        const resources = Array.from(document.images); // можно добавить fetch шрифтов и видео
        let loaded = 0;
