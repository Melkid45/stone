...
$('.works__body-information-choose button')
  .off('click')
  .on('click', function () {
...
gsap.set(
  document.querySelectorAll('.transform-item img, .transform-item video'),
  {
    yPercent: -10,
  }
);
...
document.addEventListener('DOMContentLoaded', function () {
  const workItems = document.querySelectorAll(
    '.work, .services-frame-item, .project-wrap .item, .our-services-frame .item'
  );
  ...
  function handleVideoReady(video, image, parent) {
    const isSpecialParent = parent.closest(
      '.services-frame-item, .our-services-frame'
    );
    if (isSpecialParent) {
  ...
  function initVideosForItem(item) {
    ...
    videos.forEach((video) => {
      ...
      const parentBlock = video.closest(
        '.work, .services-frame-item, .item, .project-wrap, .our-services-frame'
      );
      const image = parentBlock ? parentBlock.querySelector('img') : null;
      const isSpecialParent = parentBlock.closest(
        '.services-frame-item, .our-services-frame'
      );
